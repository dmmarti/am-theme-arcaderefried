...
// Layout Options
class UserConfig {
 </ label="Background Image", help="Select background image", options="blue,green,red,yellow", order=2 /> bg_image = "blue";
 </ label="Marquee Image", help="Choose game or default marquee", options="game,default", order=4 /> mq_image = "game";
 ...
 </ label="Default Flyer Image", help="Select default flyer image", options="blue,green,red,yellow", order=28 /> flyer_image = "blue"
 </ label="Cabinet Image", help="Choose system or default cabinet", options="system,default", order=4 /> cab_image = "system"; 
 }
...
// Default flyer image
if ( my_config["flyer_image"] == "blue") {
 local fanart = fe.add_artwork( "flyerblue", flx*0.5, 0, flw*0.5, flh );
}
if ( my_config["flyer_image"] == "green") {
 local fanart = fe.add_artwork( "flyergreen", flx*0.5, 0, flw*0.5, flh );
}
if ( my_config["flyer_image"] == "red") {
 local fanart = fe.add_artwork( "flyerred", flx*0.5, 0, flw*0.5, flh );
}
if ( my_config["flyer_image"] == "yellow") {
 local fanart = fe.add_artwork( "flyeryellow", flx*0.5, 0, flw*0.5, flh );
}
...
if ( my_config["bg_image"] == "yellow") {
 local bg = fe.add_artwork( "backyellow", 0, 0, flw, flh );
}

// Snap Image
local snap = FadeArt( "snap", flx*0.1325, fly*0.2925, flw*0.425, flh*0.475 );
...
if ( my_config["cab_image"] == "system") {
 local cab = fe.add_image( "cabinets/[DisplayName].png" 0, 0, flw, flh );
}

 
// Marquee Image

//local mq = fe.add_image( "default.png", flx*0.105, fly*0.0225, flw*0.4785, flh*0.1975 );
local mq = fe.add_image( "marquees/[DisplayName].png", flx*0.105, fly*0.0225, flw*0.4785, flh*0.1975 );

if ( my_config["mq_image"] == "game") {
 local mq = fe.add_artwork( "marquee" flx*0.105, fly*0.0225, flw*0.4785, flh*0.1975 );
}
